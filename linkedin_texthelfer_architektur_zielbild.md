...
| **AP10** | **in Arbeit** | Golden Cases: `tests/fixtures/golden-recommendation-cases.mjs` + `tests/unit/golden-recommendations.test.js`; aktuell **42 Fälle** (Feed/Baseline/Risk + `invite/headline/article`, inkl. DE/EN-Mix), Includes/Excludes/Top-3 + Anti-FP/FN (u. a. URL-Query-`?`, kurzer Frage-Text, Prefix-Excludes bei Risk). Neu im Article-Pack: `too_many_threads`, `core_claim_needs_summary`, `closing_takeaway_missing`; `verify.mjs golden`. |

**Nächster empfohlener Schritt:** AP10 fein kalibrieren (v. a. Tonalität pro Zielkontext und verbleibende Edge-Cases), danach ggf. ML-Schnittstelle (AP11).
...
- [x] Start-Set (42 Cases) mit realistischen Feed-/Risk-/Baseline-Szenarien
